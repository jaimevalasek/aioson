---
updated_at: 2026-06-02
briefings:
  - slug: fallow-quality-governance
    status: implemented
    source_plans: ["external repo: https://github.com/fallow-rs/fallow", "web research: researchs/fallow-rs-ai-agent-code-quality-2026/summary.md"]
    created_at: "2026-06-01"
    approved_at: "2026-06-02"
    prd_generated: ".aioson/context/prd-quality-governance-baseline-and-new-regression-gate.md"
  - slug: cross-tool-project-knowledge
    status: implemented
    source_plans: ["plans/cross-tool-project-knowledge-and-gemini-phaseout.md"]
    created_at: "2026-05-22"
    approved_at: "2026-05-23"
    prd_generated: null
  - slug: neural-chain
    status: implemented
    source_plans: ["conversational — seed from user idea", "2026-05-21 (uninstall-app-button incident)"]
    created_at: "2026-05-21"
    approved_at: "2026-05-21"
    prd_generated: ".aioson/context/prd-neural-chain.md"
  - slug: harness-driven-aioson
    status: implemented
    source_plans: ["plans/Harness-Driven/evolucao.txt", "plans/Harness-Driven/resumo.txt"]
    created_at: "2026-04-10"
    approved_at: "2026-04-10"
    prd_generated: ".aioson/context/prd-harness-driven-aioson.md"
  - slug: lay-user-agent-mode
    status: implemented
    source_plans: ["plans/lay-user-agent-mode.md"]
    created_at: "2026-05-16"
    approved_at: "2026-05-16"
    prd_generated: ".aioson/context/prd-lay-user-agent-mode.md"
  - slug: agent-chain-continuity-delivery-fix
    status: approved
    source_plans: ["conversational — seed from @dev diagnostic session 2026-05-16 (task-dev-1778959550617)"]
    created_at: "2026-05-16"
    approved_at: "2026-05-16"
    prd_generated: null
  - slug: cursor3-harness-evolution
    status: draft
    source_plans: ["plans/relatorio-proposta-melhoria-analisar.txt"]
    created_at: "2026-05-18"
    approved_at: null
    prd_generated: null
  - slug: workflow-handoff-integrity-1-9-2
    status: implemented
    source_plans: ["conversational — seed from dogfooding session in aioson-com after v1.9.2 release (2026-05-19)"]
    created_at: "2026-05-19"
    approved_at: "2026-05-19"
    prd_generated: null
  - slug: agent-orchestration-v2
    status: implemented
    source_plans: ["squad-searches/standalone/agent-orchestration-upgrade-20260527.md"]
    created_at: "2026-05-27"
    approved_at: "2026-05-27"
    prd_generated: ".aioson/context/prd-agent-orchestration-v2.md"
---

# Briefings Registry

| slug | status | source_plans | created | approved | prd |
|------|--------|-------------|---------|----------|-----|
| fallow-quality-governance | implemented | external repo: https://github.com/fallow-rs/fallow, web research: researchs/fallow-rs-ai-agent-code-quality-2026/summary.md | 2026-06-01 | 2026-06-02 | .aioson/context/prd-quality-governance-baseline-and-new-regression-gate.md |
| cross-tool-project-knowledge | implemented | plans/cross-tool-project-knowledge-and-gemini-phaseout.md | 2026-05-22 | 2026-05-23 | — |
| neural-chain | implemented | conversational — seed from user idea, 2026-05-21 (uninstall-app-button incident) | 2026-05-21 | 2026-05-21 | .aioson/context/prd-neural-chain.md |
| harness-driven-aioson | implemented | plans/Harness-Driven/evolucao.txt, plans/Harness-Driven/resumo.txt | 2026-04-10 | 2026-04-10 | .aioson/context/prd-harness-driven-aioson.md |
| lay-user-agent-mode | implemented | plans/lay-user-agent-mode.md | 2026-05-16 | 2026-05-16 | .aioson/context/prd-lay-user-agent-mode.md |
| agent-chain-continuity-delivery-fix | approved | conversational — seed from @dev diagnostic session 2026-05-16 (task-dev-1778959550617) | 2026-05-16 | 2026-05-16 | — |
| cursor3-harness-evolution | draft | plans/relatorio-proposta-melhoria-analisar.txt | 2026-05-18 | — | — |
| workflow-handoff-integrity-1-9-2 | implemented | conversational — seed from dogfooding session in aioson-com after v1.9.2 release (2026-05-19) | 2026-05-19 | 2026-05-19 | — |
| agent-orchestration-v2 | implemented | squad-searches/standalone/agent-orchestration-upgrade-20260527.md | 2026-05-27 | 2026-05-27 | .aioson/context/prd-agent-orchestration-v2.md |
