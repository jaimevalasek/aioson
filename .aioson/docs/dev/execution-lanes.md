...
1. `aioson execution:profiles:validate . --json` is an optional full-profile diagnostic (`--status` is read-only, `--refresh` re-probes valid cache); `aioson execution:run . --feature={slug} --preflight --json` is a read-only dry check. Neither is a manual prerequisite for starting work. An actual run automatically proves missing/expired/invalid routes in the active transitive fallback chain with the real unattended harness before its preflight, and repeats that JIT validation when the saved roles file changes before another dispatch. It never rewrites the selected model. A failed primary uses the next working saved fallback; only no working configured route becomes a decision. Preflight still refuses on a stale plan, invalid manifest, missing executable/unattended capability or unusable signature. Fix the cause; never bypass. Orchestrated workers have no AIOSON token or wall-clock cutoff. Context window, peak use, duration and activity are report-only telemetry; legacy `unit_timeout_ms` and `--unit-timeout` inputs are ignored. A lease left by a killed run is waited out on `--resume` — never delete the lock by hand.
