...
- **Dashboard recovery action.** A stopped run can retry pending units and resume from the local dashboard with concrete error context and progress notes. In bounded mode, an explicit acceptance retry sends QA findings back to DEV for one additional DEV → QA cycle. The action binds the displayed run, rejects duplicate/active requests, preserves configured models and QA, and reports startup/activity separately.

- **Orchestration context controls.** `execution:run --no-context-limit` disables AIOSON's runtime token cutoff for a run and persists across resumes without recompiling the approved plan or rerunning accepted units. `--context-limit` restores it. Native harness limits, usage collection and independent QA remain active; the dashboard, CLI help and orchestration tutorial explain the choice.
...
- **Execuções Antigravity podiam gastar horas relendo o mesmo contexto ou procurar o projeto no diretório do usuário.** O contrato agora fornece caminhos absolutos do workspace e do relatório, proíbe descoberta fora do projeto durante reparos de relatório e detecta ciclos estruturados de leitura sem progresso. A tentativa improdutiva termina de forma controlada e pode seguir pela cadeia de fallback autorizada. Reparos entre unidades entregam ao produtor somente os achados e caminhos sob sua responsabilidade, e cada nova tentativa aloca um relatório ainda não usado mesmo quando o estado histórico ficou incompleto. O dashboard também mostra a última ação estruturada e diferencia entrada reportada, cache e saída sem somar campos cuja sobreposição o harness não informa. Metadados de verificação de capability não deixam mais o resumo final alegando uma etapa invisível de integração pendente quando o plano não criou uma unidade manual de integração.

- **Autopilot stopped at its local QA retry budget.** Autopilot now defaults to continuous technical recovery, with a persisted `--until-complete` override and explicit `--bounded-recovery`/`--step` controls. Failed acceptance returns to the owning DEV and independent QA checks prior reproductions as well as current tests. Process failures preserve unresolved QA findings, reports and distinct attempt paths. OpenCode receives prompts through stdin to avoid Windows `ENAMETOOLONG`; early stdin closure no longer terminates the engine.

- **Orchestration CLI and Planner delivery.** Recovery and pricing refresh flags preserve the project path regardless of argument order. Detailed wave, context and recovery rules moved into a mandatory shipped Planner module, keeping its kernel within the existing size budget. Backend and frontend both inherit the project's DEV profile before their configured QA review.

- **Long harness prompts and shared worktrees.** Claude and Qwen receive full prompts through stdin. Large Grok tasks use its native prompt file; current Kimi Code receives a read-first instruction for the complete private task file, with cleanup after execution. DEV and QA profiles prohibit Git worktree mutations during unit execution and require read-only baseline inspection, preserving concurrent and previously completed work.

- **Recovery continuity and context choice.** Progress notebooks above12KB retain bounded initial/recent excerpts instead of being dropped; verification guidance preserves asynchronous runner IDs and exit evidence. Recompilation honors the matching active run’s explicit disabled context limit while retaining default/new-run quality and QA gates.

- **Recovery visibility and production diagnosis.** Dashboard recovery has a prominent yellow action and a repeated-correction notice; interrupted worker records no longer count as active without a running engine. Planner and recovery guidance require tracing full production payloads through their consumer, transport and file ownership, preserving verified supervisor fixes instead of repeating completed investigation.

- **Contradictory QA approvals.** A QA report declaring PASS while retaining critical/high findings now has an effective failed status. Continuous execution returns it to DEV and independent QA; bounded mode retains its acceptance gate. The original reported verdict and findings remain available for audit.

- **Wave recovery and API estimates.** Invalid DEV reports get separate report-only repair attempts (finite in bounded-recovery mode). Unambiguous dependency contract failures return to their owning producer for correction and QA. Codex Sol/Astra estimates use the verified OpenAI Standard tariff instead of OpenRouter rates; historical display recalculation preserves original records and shows a range when per-request context pricing is unknown. Per-model breakdown explains cached input and tariff sources.

- **Orchestration recovery lost discoveries and stopped on report formatting.** Bounded progress notes now carry concrete findings and next actions across fresh DEV/QA contexts. Autopilot can request a fresh, valid DEV report after a missing or malformed report or identity mismatch, with bounded or continuous recovery as selected; rejected reports never count as approvals. The dashboard distinguishes a context cutoff from a process crash.
...
- Resume unfinished workflows using the current default or explicitly configured route, preserving active specialist detours and validated progress. Restoring an old feature no longer reintroduces Analyst by default; completed historical workflows stay terminal.
- Product loads decision presentation before asking and provides options with a recommended first choice in every profile. Deyvin continuity and prompt-repair guidance no longer invent an Analyst gate. `security:audit` uses the current PRD and implementation plan, routes missing evidence to Product/Planner, and treats legacy requirements/spec as optional compatibility evidence.
