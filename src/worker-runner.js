...
// Worker input transport: small payloads travel as argv[2] (the historical contract);
// anything past the platform-safe argv budget travels through a temp file announced by
// AIOSON_WORKER_INPUT_FILE, with argv[2] carrying a tiny envelope. Windows caps the whole
// command line near 32 KB; Linux caps a single argument at 128 KB.
const WORKER_INPUT_ENV = 'AIOSON_WORKER_INPUT_FILE';
const WORKER_INPUT_ARGV_LIMIT = process.platform === 'win32' ? 24000 : 100000;
...
/**
 * Decide how a payload reaches the worker: inline argv (the historical contract)
 * or a temp file announced by AIOSON_WORKER_INPUT_FILE when the serialized payload
 * would not fit the platform argv budget. Returns { argv, env, transport, bytes, cleanup }.
 */
async function prepareWorkerInput(inputPayload, limit = WORKER_INPUT_ARGV_LIMIT) {
  const serialized = JSON.stringify(inputPayload);
  if (serialized.length <= limit) {
    return { argv: serialized, env: {}, transport: 'argv', bytes: serialized.length, cleanup: async () => {} };
  }
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'aioson-worker-input-'));
  const file = path.join(dir, 'input.json');
  await fs.writeFile(file, serialized, 'utf8');
  return {
    argv: JSON.stringify({ $inputFile: file }),
    env: { [WORKER_INPUT_ENV]: file },
    transport: 'file',
    bytes: serialized.length,
    cleanup: async () => { await fs.rm(dir, { recursive: true, force: true }).catch(() => {}); }
  };
}

function spawnWorker(scriptPath, inputPayload, env, timeoutMs) {
  return prepareWorkerInput(inputPayload).then((input) => new Promise((resolve) => {
    const ext = path.extname(scriptPath);
    const cmd = ext === '.py' ? 'python3' : 'node';
    const args = [scriptPath, input.argv];
    const child = spawn(cmd, args, {
      env: { ...process.env, ...env, ...input.env },
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        resolve({ ok: true, output, stderr: stderr.trim() || null, inputTransport: input.transport });
      } else {
        let errorDetail;
        try {
    ...
    child.on('error', (err) => {
      ...
      if (err.code === 'ETIMEDOUT' || err.killed) {
        ...
        // A spawn failure (ENOENT, ENAMETOOLONG, EACCES) is a worker result, not a CLI crash.
        resolve({ ok: false, error: `Worker could not be spawned: ${err.code || err.message}`, code: -1, spawnError: err.code || null });
      }
    });
  }));
}
...
async function runWorker(projectDir, squadSlug, workerSlug, inputPayload, options = {}) {
  ...
  if (agentMemory && inputPayload) {
    // Inject into _agent_memory field (readable by Node.js workers via the input contract)
...
// Input contract: small payloads arrive as argv[2]; large ones arrive through the file
// named by AIOSON_WORKER_INPUT_FILE (argv[2] then carries { "$inputFile": path }).
const input = readWorkerInput();

function readWorkerInput() {
  const file = process.env.AIOSON_WORKER_INPUT_FILE;
  if (file) return JSON.parse(require('node:fs').readFileSync(file, 'utf8'));
