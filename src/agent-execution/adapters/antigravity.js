...
// inherit agy's five-minute print timeout for longer AIOSON lanes. AGY treats
// `0ms` as an immediate successful partial result, not as unlimited, so the
// engine's zero/no-limit value is translated to Go's maximum Duration
// (roughly 292 years). Stream output also exposes provider usage when AGY
// reports it.
// Contract: https://antigravity.google/docs/cli/headless/
module.exports = createAdapter('antigravity', input => ({
  args: [
    '--input-format', 'stream-json', '--output-format', 'stream-json',
    '--print-timeout', Number(input.timeout) > 0 ? `${input.timeout}ms` : '2562047h47m16s',
    ...(input.sandbox_args || []),
