...
const {loadManifest,resolveExecutionEntry,resolveExecutionTarget}=require('./manifest');
...
async function renewLease(lease){if(!lease)return false;let current;try{current=JSON.parse(await fs.readFile(lease.file,'utf8'))}catch(e){if(e.code==='ENOENT'||e.name==='SyntaxError')return false;throw e}if(current.owner!==lease.owner)return false;await atomic(lease.file,{owner:lease.owner,expires_at:Date.now()+LEASE_MS});return true}
...
async function dispatch({projectDir,feature,agent='dev',lane=null,runId,promptPath,adapterRegistry=adapters,catalogLoader,timeout=600000}){
 ...
 const renewal=setInterval(()=>{renewLease(lease).catch(()=>{})},Math.floor(LEASE_MS/3));renewal.unref?.();
