...
Probes whether a `(host, model, effort)` combination actually works on this machine — CLI installed, login valid, model id accepted, effort supported — and records the result in `~/.aioson/hosts/signatures.json` (override: `AIOSON_HOST_SIGNATURES`) with a TTL (default 24h). The probe uses the exact non-interactive argv of the execution adapter in provider read-only mode, inside an empty temporary directory, with a one-word prompt; it never touches a project. Hosts come from the single registry behind `tool:capabilities` — Antigravity, Claude Code, Codex, Grok, Kimi Code, OpenCode and Qwen Code are dispatchable; Muse remains interactive-only. The logical `antigravity` host resolves to `agy`, never the Electron editor launcher. A host without a registered read-only flag (OpenCode or Antigravity) is probed without that precaution (`probe.sandbox: none`) instead of being kept out of the lanes. After the read-only probe passes, an **unattended write probe** runs the exact unattended `workspace-write` argv a lane worker gets in another empty temporary directory and asks for one file: written → `verified`, exited without it → `unverified` (valid; the run's preflight warns), alive past the probe budget → `blocked`, refused → `failed` — the last two invalidate the signature with `host_not_unattended`. The result is kept under `unattended.yolo`; `--unattended-probe=false` skips the write probe and keeps what an earlier one proved.
...
Answers whether the orchestrated execution path (compiled lanes as parallel external processes, one host/model per role) is available in this project on this machine: `.aioson/config/execution-roles.json` present, valid and enabled — the unlock file the planner seeds disabled (`execution:seed`) and a person or the supervising desktop client enables — every role still at the default model confirmed, **and** every declared role signed (`host:signature`) and unexpired. `available:false` carries `reason`, `pending_confirmation[]` and the exact onboarding action. With `--feature` it also describes the compiled plan, measured scale and recommendation. The `execution` block exposes spawner support/configuration plus `context_limit_enabled:false` and `time_limit_enabled:false`; AIOSON reports context use and duration but does not terminate orchestrated workers for either. Always exits 0 — it is a question, not a gate. See [Agent execution — Orchestrated execution](agent-execution.md#orchestrated-execution-roles-offer-compile).
...
Writes `.aioson/config/execution-roles.json` for the lanes given — or, with `--feature` and no `--lanes`, for the plan's `## Development execution lanes` table, or, before that table exists, one lane per measured surface of a two-surface plan (`lanes_source: surfaces`) — **disabled**: one `{lane}_dev` role per lane plus `integration_dev` and `qa`, each on an execution host installed on this machine (the reviewer on a second host when there is one, `independent_review: false` otherwise), every model the harness default `configured-default`, `source` naming the planner and the feature. Never touches an existing file (`already_present`, with `missing_roles` the lanes need and the file lacks — exit 0). Refuses with the cause when no execution host is installed (`no_execution_host`, with the install command per registered host), when the write fails (`write_failed`), or when the lanes are missing or malformed (`lanes_required`, `lane_invalid`, `too_many_lanes`) — exit 1. Choosing the models, enabling the file and signing the hosts stay the owner's acts; the framework seeds, it never unlocks.

## execution:profiles:validate

```bash
aioson execution:profiles:validate [path] [--profile=<name[,name2]>] [--concurrency=<1..10>] [--ttl=<hours>] [--timeout=<ms>] [--refresh] [--status] [--json]
```

Validates every unique host/model/effort route in the enabled execution profiles through the same read-only plus unattended-write harness as `host:signature`. Missing, expired and invalid routes are probed in parallel (three at once by default); valid cached signatures are reused unless `--refresh` is set. Probe results are merged atomically into the machine signature store. `routing` reports whether every role in the active profile has a valid primary or transitive fallback; `routes` preserves every configured binding and its evidence. `--status` starts no harness. Exit 0 only when active routing is ready and every selected enabled route is valid; otherwise exit 1 with the failed alternatives still visible. This command is an optional full-profile diagnostic: an actual `execution:run` performs the required active-chain validation automatically.
...
Runs the compiled plan by readiness, with each unit as a `dev → qa` pipeline and up to 10 disjoint unit pipelines at once (`parallel.max_concurrent_lanes` is the backward-compatible field name). Several units from one logical lane may overlap. Real dependencies and file conflicts still gate launch. The saved execution-role configuration is live: before starting, and whenever that file changes before a later stage, the engine JIT-probes missing/expired/invalid routes in the active transitive fallback chain, then reloads the active profile, model, effort, fallbacks and independent-QA policy; the scheduler reloads pool capacity. Validation never rewrites the selected model. A failed primary probe advances to a working saved fallback, so the run pauses only when no configured route works. These edits do not stale the compiled plan. `--preflight` is deliberately read-only and starts no harness. If `integration_dev` is configured before the final boundary, one final correcting integration DEV runs the complete verification after all units, followed by shared independent QA. A process already in flight retains its launch identity; the next dispatch obeys the saved choice. The run holds the feature lease, persists every transition and resumes idempotently. AIOSON imposes no token or wall-clock deadline; context use, elapsed time and file activity remain report-only telemetry.

Convergence is bounded by behavior, not by elapsed time: four identical structured actions or 24 consecutive read-only actions stop an unproductive attempt; one equivalent no-progress retry is automatic, then the circuit opens; and QA may return a unit to DEV at most five times. A sixth failed review becomes `decision_required` with `dev_qa_cycle_limit`, preserving all findings for diagnosis. Cross-unit contract repair routes to the unique compiled owner in the same or an earlier wave and coalesces duplicate pending repairs so consumers wait for one corrected producer. Scope drift, silence, disk activity and reports remain measured and visible. Live lines go to stdout (stderr with `--json`), and `execution:status --watch` remains the durable view when a wrapper swallows process output.
