...
| `execution:seed` | Grava `.aioson/config/execution-roles.json` **desligado** para as faixas dadas (`--lanes=a,b`, a tabela de faixas do plano com `--feature`, ou — antes da tabela existir — uma faixa por superfície medida de um plano backend+frontend): um `{lane}_dev` por faixa mais `integration_dev` e `qa`, cada um numa CLI de execução instalada na máquina (revisor em outra CLI quando há mais de uma), no modelo padrão do harness. Nunca altera arquivo existente (`already_present` nomeia os papéis que faltam); recusa nomeando a causa sem CLI instalada ou quando a gravação falha | Pelo planner ao fechar um plano com faixas. Escolher modelo, ligar e assinar são atos do dono. Aceita `--lanes`, `--feature`, `--json` |
| `execution:profiles:validate` | Diagnóstico explícito em lote de cada rota host/modelo/effort única dos perfis habilitados com as mesmas sondas real e não assistida de `host:signature`; reaproveita cache válido, testa ausentes/expiradas/inválidas em paralelo e mescla as assinaturas uma vez. `routing.ready` mostra se cada papel ativo tem rota principal ou fallback transitivo válido; `--status` não inicia harness e `--refresh` testa tudo novamente | Opcional para inspecionar todos os perfis; o run real valida automaticamente a cadeia ativa. Aceita `--profile`, `--concurrency=1..10`, `--ttl`, `--timeout`, `--refresh`, `--status`, `--json` |
