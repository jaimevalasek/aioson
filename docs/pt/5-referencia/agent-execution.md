...
Os hosts vêm de um registro único (`src/lib/tool-capabilities.js`, exposto por `aioson tool:capabilities --json`): Claude Code, Codex, Grok, Kimi Code, OpenCode e Qwen Code são despacháveis (`listExecutionHosts()`); Muse e Antigravity (`agy`) são conhecidos só pela superfície interativa até um adaptador provar o contrato não-interativo deles. Um host novo precisa de adaptador para manter resolução de executável, capabilities, argumentos, redação e telemetria em modo fail-closed.
...
- Recusas são determinísticas a partir do registro: `unknown_host`, `unsupported_host_execution` (host só interativo — Muse, Antigravity), `effort_unsupported_by_host`, `invalid_reasoning_effort`, `permission_mode_unsupported` (sem flag não assistido registrado — nenhum hoje). Host sem flag read-only (OpenCode) é sondado sem essa precaução (`probe.sandbox: none`), nunca recusado por isso; a sonda de escrita não assistida roda igual.
